...
export default function AddFriendModal({ show, set }: { show: boolean, set: Dispatch<SetStateAction<boolean>> }) {
    ...
    return (
        <form onSubmit={(event) => handleSubmit(event)} id="backdrop" className="absolute w-full h-full left-0 top-0 bg-[rgba(0,0,0,0.75)] z-[999] flex justify-center items-center">
            <div className="w-64 p-4 bg-stone-900 flex flex-col gap-2 rounded-lg">
                ...
                <div className="flex justify-end gap-2">
                    <Button type={"button"} onClick={() => set(false)} className="bg-stone-800 p-1 rounded-lg">Cancel</Button>
                    <Button type={"submit"} className="bg-stone-800 p-1 rounded-lg">Send</Button>
                </div>
            </div>
        </form>
    )
